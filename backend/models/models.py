...
class OpeningBalance(db.Model):
    ...
    amount = db.Column(db.Float, nullable=False)        # keep Float to match legacy
    date = db.Column(db.String(50), nullable=False)     # e.g. '2025-10-08'

class Transaction(db.Model):
    __tablename__ = 'transactions'
    txn_id = db.Column(db.Integer, primary_key=True)
    date = db.Column(db.String(50), nullable=False)     # ISO 'YYYY-MM-DD' recommended
...
class Budget(db.Model):
    ...
    amount = db.Column(db.Float, nullable=False)
    period = db.Column(db.String(20), nullable=False)   # e.g. '2025-10'
